...
export default class AppiumBoostPlugin extends BasePlugin {
  ...
  boostDiscover (opts = {}) {
    if (!this.poweredUP) {
      // move "require" into method since starts scanning as soon its required
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeoutPromise(timeout, '', { signal })
        .then(() => {
        ...
        .catch((err) => {
          if (err.name === 'AbortError') {
            console.debug('Hub found: Abort discover timeout');
          } else {
            console.warn(err);
          }
        });
      ...
      this.poweredUP.on('discover', (hub) => {
        console.log(`Hub discovered: ${hub.name}!`);
        ac.abort();
